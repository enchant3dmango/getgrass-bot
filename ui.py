            ...
            async with proxy_connect(
                ...
                async def send_ping():
                    while True:
                        ...
                        insert_log(
                            log_widget,
                            f"{get_datetime()} send_message: {send_message}\n",
                            "info",
                        )
                ...
                while True:
                    ...
                    logger.info(message)
                    insert_log(
                        log_widget, f"{get_datetime()} message: {message}\n", "info"
                    )
                    ...
                    if message.get("action") == "AUTH":
                        ...
                        insert_log(
                            log_widget,
                            f"{get_datetime()} auth_response: {auth_response}\n",
                            "info",
                        )
                        ...
                        insert_log(
                            log_widget,
                            f"{get_datetime()} pong_response: {pong_response}\n",
                            "info",
                        )
...
def start_operation():
    ...
    asyncio.run_coroutine_threadsafe(
        connect_to_wss(user_id, proxy_list, log_box), new_loop
    )
...
def open_github(event):
    url = "https://github.com/enchant3dmango"
    subprocess.Popen(["start", url], shell=True)


def open_referral(event):
    url = "https://app.getgrass.io/register/?referralCode=SxjVUVv2xt1LNCp"
    subprocess.Popen(["start", url], shell=True)
...
if __name__ == "__main__":
    ...
    github_label = tk.Label(
        labels_frame,
        text="GitHub Repository: https://github.com/enchant3dmango",
        fg="blue",
    )
    github_label.pack(side=tk.LEFT)

    referral_label = tk.Label(
        labels_frame,
        text="Referral Link: https://app.getgrass.io/register/?referralCode=SxjVUVv2xt1LNCp",
        fg="red",
    )
    ...
    labels_frame.pack()
    log_box.insert(tk.END, "Please enter the correct user_id as prompted.\n", "info")
    log_box.insert(
        tk.END,
        "The format of Socks5 Proxy is socks5://username:password@ip:port\n",
        "info",
    )

    asyncio_loop_thread = threading.Thread(
        target=run_asyncio_loop, args=(new_loop,), daemon=True
    )
